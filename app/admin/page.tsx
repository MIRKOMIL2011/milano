...
export default function AdminPage() {
  ...
  const updateOrderStatus = async (orderId: number, newStatus: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const newTab = getTabForStatus(newStatus)
        if (newTab !== activeTab) {
          setActiveTab(newTab)
        }
        ...
        toast({
          title: t("success"),
          description: `Buyurtma holati "${getStatusText(newStatus)}" ga o'zgartirildi`,
        })
  ...
  // Helper function to determine which tab a status belongs to
  ...
  // Fixed filtering logic with better organization
  const filterOrdersByStatus = (tabName: string) => {
    switch (tabName) {
      case "pending":
...
// Orders List Component with improved status handling
function OrdersList({
  orders,
  onUpdateStatus,
  onViewOrder,
